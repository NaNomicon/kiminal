...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText('Timesheets'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/timesheets' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })

  it('navigates for a second top-level nav item', async () => {
    const screen = await renderWithSearchProvider()
