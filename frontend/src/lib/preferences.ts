...
export function prefValue(preferences: UserPreference[], name: string): string | null | undefined {
  return preferences.find((p) => p.name === name)?.value
}

/** Maps a boolean-valued preference to a boolean, defaulting when absent. */
export function prefBool(preferences: UserPreference[], name: string, fallback: boolean): boolean {
