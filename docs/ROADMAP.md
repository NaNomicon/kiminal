...
- **Token lifecycle** — API tokens never expire, and signing out in the SPA only clears the client cookie; it does not revoke the server token. The user CAN revoke individual tokens via the Settings token manager (list `GET /api/users/api-token` returns each token's DB id, and `DELETE /api/users/api-token/{id}` revokes by that id), so a known-stolen token can be revoked manually. The real gaps are: (a) a stolen token remains valid until the victim notices and revokes it — there is no expiry and no "revoke all my tokens / sign out everywhere" action, and (b) the victim may not know which token leaked. Token expiry and a revoke-all-for-user action are deferred to the TS backend port (see the deviation note in "Backend migration"). Token entropy is already 32-byte (256-bit) in the PHP routes.
